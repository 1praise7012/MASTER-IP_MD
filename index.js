...
async function startsPairing(VranCe) {
  ...
  try {
    console.log(chalk.yellow.bold('[ SYSTEM ] verifying user...'))

    await multiAuthState(VranCe, PaiCode, userName)

    console.log(chalk.green.bold('[ OK ] Pairing your bot...'))

  } catch (err) {
    console.log(
      chalk.red.bold('[ ERROR ] unable to send you pairing: ') +
      err.message
    )
    process.exit(1)
  }
...
async function startWhatsAppBot() {
  ...
  if (!VranCe.authState.creds.registered) {
    ...
    while (!isAuthorized) {
      console.log(chalk.red.bold('Enter your number with your country code,\nexample : 26378xxx'));
  ...
  VranCe.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      console.log(chalk.blue.bold(`[ ${jam} WIB ] MASTER-IP_MD connected success...`))

      // ===== NEW FEATURE: AUTO ONLINE HACKER PANEL MESSAGE =====
      try {
...
║  ENGINE         : Baileys MD
║  VERSION        : ${global.version}
║  RUNTIME        : ${Math.floor(process.uptime() / 3600)}h ${Math.floor(process.uptime() / 60) % 60}m ${Math.floor(process.uptime() % 60)}s}
║  TIME           : ${jam}
╠═══════════════════════╣
║  CREATED BY     : ${global.ownername}
║  MODE           : PUBLIC
╚═══════════════════════╝
...
⚠ Official Master-IP Bot Is Now Active ⚠
`
        await VranCe.sendMessage(botNumber, { text: onlineMessage })

      } catch (err) {
        console.log("Failed to send auto online message: ", err)
      }
